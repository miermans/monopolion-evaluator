...
parser.add_argument('--training_data', metavar='TRAINING_DATA', type=str, required=True,
                    help="Path to training data file, encoded using ProtoBuf")
parser.add_argument('--validation_data', metavar='VALIDATION_DATA', type=str,
...
def main(args=None):
    ...
    training_df = to_data_frame(parse_delimited_file(args.training_data))
    validation_df = None
    if args.validation_data is not None:
        validation_df = to_data_frame(parse_delimited_file(args.validation_data))
